...
Note that breakpoint debugging in `wrangler dev` using `--remote` could extend Worker CPU time and incur additional costs. It's recommended to use `wrangler dev` in local mode by specifying no `--remote` option or with `--local`.
